...
void letra (unsigned char k, int x, int y)
{
  ...
  switch (k)
    ...
    case 'B':
    	if(!estadoAnimacionFuctionMOV()){
        	movimientoManoDerechaMOV();
        }
        break;
    case 'b':
    	if(!estadoAnimacionFuctionMOV()){
		movimientoManoIzquierdaMOV();
	}
	break;
    case 'V':
    	if(!estadoAnimacionFuctionMOV()){
		movimientoDedosArribaMOV();
	}
	break;
    case 'v':
    	if(!estadoAnimacionFuctionMOV()){
		movimientoDedosAbajoMOV();
	}
	break;
    case 'A':
    
    	if(estadoAnimacionFuctionMOV()){
	 	setAnimacionMOV(false);
	 	cout<<"*************************************************************************"<<endl;
	 }
	 else{
	 	setAnimacionMOV(true);
	 	cout<<"++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<endl;
	 }
	break;
